...
function renderDataBrokerBG(data, colors, path) {
    ...
    for (let i = 0; i < data.length; i++) {
        if (data[i].function === "BrokerField") { // cover area of broker
            // rounding and adding/substracting of 0.05 counteracts overlap
            const rect = createGeoJsonPolygon([ [ [Math.round(data[i].boxMinX)+0.05, Math.round(data[i].boxMinY)+0.05],
                [Math.round(data[i].boxMinX)+0.05, Math.round(data[i].boxMaxY)-0.05],
                [Math.round(data[i].boxMaxX)+0.1, Math.round(data[i].boxMaxY)+0.1],
                [Math.round(data[i].boxMaxX)-0.05, Math.round(data[i].boxMaxY)-0.05],
                [Math.round(data[i].boxMaxX)-0.05, Math.round(data[i].boxMinY)+0.05],    
                [Math.round(data[i].boxMaxX)-0.1, Math.round(data[i].boxMinY)-0.1],
                [Math.round(data[i].boxMinX)+0.05, Math.round(data[i].boxMinY)+0.05] ] ]);

            // select color
            const color = last_color;
            const id = "rect_"+i;

            // generate point if it does not exist
            let rect_svg = d3.select("#"+id);
            if (rect_svg.empty()) {
                rect_svg = data_group.append("path")
                    .attr("id", id);
            }
            ...
            rect_svg
                ...
                .datum(rect)
                .attr("d", path);
        } else if (data[i].function === "Broker") { // new broker
            ...
            if (point_svg.empty()) {
                point_svg = broker_group.append("path")
                    .attr("id", id);
            }
...
function renderDataBrokerGQPS(data, colors, path) {
    ...
    for (let i = 0; i < data.length; i++) {
        if (data[i].function === "Broker") { // new broker
            const point = createGeoJsonPoint([data[i].lon, data[i].lat]); // lon, lat; has to be inverted because of GeoJson
            ...
            if (renderStateGQPS.highlight !== null && renderStateGQPS.highlight !== undefined) {
                if (renderStateGQPS.highlight.includes(id)) {
                    color = highlight_color;
                }
                if (renderStateGQPS.highlight[renderStateGQPS.highlight.length-1] === id) {
            ...
            if (point_svg_click.empty()) {
                point_svg_click = broker_group.append("path")
                    .attr("id", id+"_click");
            }
...
function renderDataBrokerDHT(data, colors, path) {
    // process data
    if (renderStateDHT.brokerToTopicCount === null || renderStateDHT.brokerToTopicCount === undefined || renderStateDHT.maxTopics === null || renderStateDHT.maxTopics === undefined) {
    ...
    for (let i = 0; i < data.length; i++) {
        if (data[i].function === "Broker") { // new broker
            ...
            let color_no = Math.floor(renderStateDHT.brokerToTopicCount[i] / renderStateDHT.maxTopics * colormapDHT.length);
            if (color_no >= colormapDHT.length) {
                color_no = colormapDHT.length -1; // clip to last available color
            }
            let color = colormapDHT[color_no];

            if (renderStateDHT.highlight !== null && renderStateDHT.highlight !== undefined) {
                if (renderStateDHT.highlight === id) {
                    color = highlight_color;
                    d3.select("#"+id+"_table").style("background-color", highlight_color);
                }
            }
            ...
            point_svg_click
                ...
                .on("click", function() {
                    const id = this.id.substring(0, this.id.length - 6); // chop off the "_click" of id
                    if (renderStateDHT.highlight !== null && renderStateDHT.highlight !== undefined) {
                        d3.select("#"+renderStateDHT.highlight+"_table").style("background-color", "initial");
                        if (renderStateDHT.highlight === id) {
