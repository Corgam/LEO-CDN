...
(async function () {
  ...
  function localDataListener(event) {
    ...
    function parseFile() {
      ...
      file_menu.select("#local-data-drop").property("disabled", false);
  ...
  document
    .getElementById("local-data")
    .addEventListener("change", localDataListener);
  ...
  const broker_bg = await fetch("CSV/input-BrokerBG_brokers.csv")
    ...
    .then(parseResponse);
  const broker_dis_gb_events = await fetch(
    "CSV/input-BrokerDisGBEvents_brokers.csv"
  )
    .then(getResponseText)
    .then(parseResponse);
  const broker_dis_gb_subscriptions = await fetch(
    "CSV/input-BrokerDisGBSubscriptions_brokers.csv"
  )
    .then(getResponseText)
    .then(parseResponse);
  const broker_flooding_events = await fetch(
    "CSV/input-BrokerFloodingEvents_brokers.csv"
  )
    .then(getResponseText)
    .then(parseResponse);
  const broker_flooding_subscriptions = await fetch(
    "CSV/input-BrokerFloodingSubscriptions_brokers.csv"
  )
    .then(getResponseText)
    .then(parseResponse);
  ...
  if (broker_bg != null) {
    file_menu.select("#broker-bg").property("disabled", false);
  }
  if (broker_dis_gb_events != null) {
    file_menu.select("#broker-dis-gb-events").property("disabled", false);
  }
  if (broker_dis_gb_subscriptions != null) {
    file_menu
      .select("#broker-dis-gb-subscriptions")
      .property("disabled", false);
  }
  if (broker_flooding_events != null) {
    file_menu.select("#broker-flooding-events").property("disabled", false);
  }
  if (broker_flooding_subscriptions != null) {
    file_menu
      .select("#broker-flooding-subscriptions")
      .property("disabled", false);
  }
  if (broker_gqps != null) {
    file_menu.select("#broker-gqps").property("disabled", false);
  }
  if (broker_dht != null) {
    file_menu.select("#broker-dht").property("disabled", false);
  }
  ...
  function createPathAndPrepSvg(projection, width) {
    ...
    map_svg
      .call(
        drag(projection)
          .on("drag.render", () =>
            render(land110, broker_data, broker_data_type, path)
          )
          .on("end.render", () =>
  ...
  function widthListener() {
    ...
    document.getElementById("visualization-container").style.width =
      width + "px";
  ...
  document
    .getElementById("btn-hide-dht-table")
    .addEventListener("click", dhtTableListener);
