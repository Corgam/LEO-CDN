    ...
    if not success:
        ...
        try:
            ...
            for node in nodes:
                ...
                data = {"node": fred, "satellite": name, "keygroup": "manage"}
                r = requests.post(
                    url=base_url + "/addToKeygroup/manage", data=json.dumps(data)
                )
                if r.status_code == 200:
                    print("Should be done?")
                    fred_client.node_to_keygroup("manage", fred)
                    append_data(
                        "manage", "addresses", "http://" + ip + ":" + str(port) + "/"
                    )
                    break
    else:
        fred_client.set_data(
            "manage", "addresses", json.dumps(["http://" + ip + ":" + str(port) + "/"])
        )
...
def addKeygroup():
    ...
    if fred_client.add_node_to_keygroup(
        data["keygroup"], data["fred"], data["satellite"]
    ):
